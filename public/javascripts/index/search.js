document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        arniCategories = categoriesDataEl ? JSON.parse(categoriesDataEl.textContent || '[]') : [];
    } catch (error) {
    ...
    async function fetchProducts() {
        ...
        try {
            ...
            currentFilters.categories.forEach((category) => queryParams.append('categories', category));
    ...
    async function getSearchRecommendations() {
        ...
        try {
            ...
            return Array.isArray(suggestions)
                ? suggestions.map((suggestion) => decodeURIComponent(String(suggestion)))
                : [];
        } catch (error) {
            console.error('Error:', error);
            return [];
    ...
    function initializeSearchSuggestions() {
        ...
        function positionSuggestions() {
            ...
            const containerRect = searchInput.closest('.search-container')?.getBoundingClientRect() || rect;
            ...
            suggestionsContainer.style.setProperty('--suggestions-width', `${width}px`);
        }
    ...
    function initializeFilters() {
        ...
        categoryCheckboxes.forEach((checkbox) => {
            checkbox.addEventListener('change', async (event) => {
                ...
                currentFilters.categories = event.target.checked ? [event.target.value] : [];
                currentPage = 1;
                await refreshProducts();
                closeFilters();
            });
    ...
    function updateURLParameters() {
        ...
        if (currentFilters.sort !== 'popularity') url.searchParams.set('sort', currentFilters.sort);
        else url.searchParams.delete('sort');

        url.searchParams.delete('categories');
        currentFilters.categories.forEach((category) => url.searchParams.append('categories', category));
        ...
        currentFilters.sizes.forEach((size) => url.searchParams.append('sizes', size));
    ...
    document.querySelectorAll('.search-section, .filters, .results-toolbar, .footer-brand, .footer-col').forEach((element, index) => {
