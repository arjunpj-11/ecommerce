...
function createProductCard(product) {
  const discount = Math.round(
    (1 - product.productDetails.discountPrice / product.productDetails.price) *
      100,
  );
  const rating = Number(product.productDetails.review) || 0;
...
function renderPagination() {
  ...
  let paginationHTML = `
        <button type="button" class="page-btn" data-page="${currentPage - 1}" ${currentPage === 1 ? "disabled" : ""} aria-label="Previous page">
  ...
  for (let i = 1; i <= totalPages; i++) {
    if (
      ...
      paginationHTML += `
                <button type="button" class="page-btn ${currentPage === i ? "active" : ""}" data-page="${i}">
                    ${i}
                </button>
            `;
  ...
  paginationHTML += `
        <button type="button" class="page-btn" data-page="${currentPage + 1}" ${currentPage === totalPages ? "disabled" : ""} aria-label="Next page">
    ...
    if (products.length === 0) {
      productGrid.innerHTML = `
                <div class="empty-state" role="status">
                    <div class="empty-state-icon" aria-hidden="true"><i class="fas fa-search"></i></div>
                    <h2>No matching products</h2>
                    <p>Try a broader search term or clear some filters to see more products.</p>
                    <a href="/shop" class="empty-state-action">Browse all products</a>
                </div>`;
      return;
    }
