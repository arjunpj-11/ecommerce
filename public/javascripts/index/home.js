document.addEventListener("DOMContentLoaded", function () {
  ...
  function initializeScrollAnimations() {
    ...
    document
      .querySelectorAll(".section-title, .category, .product-card")
      .forEach((el) => {
        el.style.opacity = "0";
  ...
  function initializePerformanceOptimizations() {
    window.addEventListener("load", () => {
      const imagesToPreload = [
        ...
        ...Array.from(document.querySelectorAll(".category img")).map(
          (img) => img.src,
        ),
        ...Array.from(document.querySelectorAll(".product-image")).map(
          (img) => img.src,
        ),
      ].filter(Boolean);
