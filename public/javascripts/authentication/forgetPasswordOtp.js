document.addEventListener('DOMContentLoaded', () => {
  ...
  async function handleResendOtp() {
    const count = Number.parseInt(localStorage.getItem(STORAGE_COUNT_KEY) || '0', 10);
  ...
  function startTimer() {
    ...
    const count = Number.parseInt(localStorage.getItem(STORAGE_COUNT_KEY) || '0', 10);
    const expiryTime = Number.parseInt(localStorage.getItem(STORAGE_EXPIRY_KEY) || '0', 10);
  ...
  function setExpiryTime() {
    localStorage.setItem(STORAGE_EXPIRY_KEY, String(Date.now() + LOCK_DURATION_MS));
  }
