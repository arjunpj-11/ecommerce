document.addEventListener('DOMContentLoaded', () => {
  ...
  async function removeItem(event, variantId) {
    ...
    try {
      ...
      const item = document.querySelector(`.cart-item[data-variant-id="${cssEscape(variantId)}"]`);
      item?.remove();
  ...
  async function applyCoupon() {
    ...
    try {
      ...
      if (!response.ok) {
        showToast(result?.error || result?.message || 'Invalid coupon code.', 'error');
        return;
      }
  ...
  function updateCartTotals() {
    ...
    } else if (couponStatus && couponStatus.textContent !== 'No discount applied') {
  ...
  function calculateSubtotal() {
    ...
    document.querySelectorAll('.cart-item').forEach((item) => {
      const quantity = Number.parseInt(item.querySelector('.quantity-input')?.value || '0', 10);
