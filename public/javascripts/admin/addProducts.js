...
class VariantImageManager {
  ...
  initElements() {
    this.miniBoxesContainer = document.getElementById(
      `miniBoxes_${this.variantId}`,
    );
    this.previewContainer = document.getElementById(
      `previewContainer_${this.variantId}`,
    );
  ...
  initEventListeners() {
    if (this.editModal) {
      document
        .getElementById(`modalClose_${this.variantId}`)
        .addEventListener("click", () => this.closeModal());
      document
        .getElementById(`cancelBtn_${this.variantId}`)
        .addEventListener("click", () => this.closeModal());
    }
    if (this.imageInput) {
  ...
  createModalIfNeeded() {
    if (!this.editModal) {
      const template = document
        .getElementById("editModalTemplate")
...
class ProductManager {
  ...
  initColorPickerEvents() {
    ...
    document.getElementById("cancelColorPicker").onclick = () =>
      modal.classList.remove("active");
        ...
        (variant) => `
            <div class="variant-color" id="variant_${variant.id}">
                <div class="color-header">
                    ...
                    <button type="button" class="remove-variant" data-product-action="remove-variant" data-variant-id="${variant.id}">Remove</button>
                </div>
  ...
  validateForm() {
    ...
    if (this.variants.length === 0) {
      ...
      this.variants.forEach((variant, index) => {
        ...
        const imageCount = variantManager.images.filter(
          (img) => img !== null,
        ).length;

        if (imageCount < 3) {
          errors.push(
            `Color ${variant.color} requires at least 3 images (currently has ${imageCount})`,
          );
        }
        ...
        if (!hasStock) {
          errors.push(
            `Color ${variant.color} requires stock quantity for at least one size`,
          );
        }
      });
    }
  ...
  showErrorMessage(errors) {
    ...
    if (submitButton && submitButton.parentNode) {
      ...
      submitButton.parentNode.insertBefore(
        errorContainer,
        submitButton.parentNode.firstChild,
      );
    } else {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (button.dataset.productAction === "remove-variant") {
