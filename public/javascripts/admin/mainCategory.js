...
const catModal = {
  ...
  async handleSubmit(e) {
    ...
    try {
      ...
      const response = await fetch("/admin/maincategories/create", {
        method: "POST",
        headers: {
...
document.addEventListener("click", (e) => {
  if (window.innerWidth <= 768) {
    if (
      ...
      !e.target.closest(".cat-modal__container")
    ) {
      sidebar?.classList.remove("sidebar-visible");
      overlay?.classList.remove("overlay-visible");
    }
  ...
  search?.addEventListener("input", async () => {
    ...
    try {
      const response = await fetch(
        `/admin/mainCategories/search?value=${encodeURIComponent(searchValue)}`,
        {
          method: "GET",
        },
      );
  ...
  document.querySelectorAll(".makeInactive").forEach((btn) => {
    btn.addEventListener("click", async () => {
      try {
        ...
        const response = await fetch("/admin/maincategories/inactivate", {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
  ...
  document.querySelectorAll(".makeActive").forEach((btn) => {
    btn.addEventListener("click", async () => {
      try {
        ...
        const response = await fetch("/admin/maincategories/activate", {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ id }),
        });
  ...
  document.querySelectorAll(".permanentDelete").forEach((btn) => {
    btn.addEventListener("click", async () => {
      try {
        ...
        const response = await fetch("/admin/mainCategories/delete", {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ id }),
        });
...
function initializeEditForm(form, btn) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      const response = await fetch("/admin/mainCategories/edit", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
async function fetchTableData() {
  try {
    const response = await fetch("/admin/mainCategories/table", {
      method: "POST",
    });
    if (!response.ok) {
      throw new Error(`Network response was not ok: ${response.statusText}`);
