  ...
  async function fetchProductsTable(page = 1, search = "") {
    try {
      const response = await fetch(
        `/admin/products/table?page=${page}&search=${encodeURIComponent(search)}`,
        {
          method: "POST",
        },
      );
      ...
      const paginationHtml = html.match(
      ...
      form.addEventListener("submit", async (e) => {
        ...
        try {
          ...
          if (result.success) {
            ...
            // Refresh the table
            const currentPage =
              document.querySelector(".pagination-btn.active")?.dataset.page ||
              1;
            const searchQuery =
