class addVariantImageManager {
  ...
  initElements() {
    this.miniBoxesContainer = document.getElementById(
      `add-miniBoxes_${this.variantId}`,
    );
    this.previewContainer = document.getElementById(
      `add-previewContainer_${this.variantId}`,
    );
    this.editModal = document.getElementById(`add-editModal_${this.variantId}`);
    this.uploadForm = document.getElementById(
      `add-uploadForm_${this.variantId}`,
    );
    this.imageInput = document.getElementById(
      `add-imageInput_${this.variantId}`,
    );
    this.cropContainer = document.getElementById(
      `add-cropContainer_${this.variantId}`,
    );
    this.cropArea = document.getElementById(`add-cropArea_${this.variantId}`);
    this.cropPreview = document.getElementById(
      `add-cropPreview_${this.variantId}`,
    );
    this.previewMini = document.getElementById(
      `add-previewMini_${this.variantId}`,
    );
    this.previewLarge = document.getElementById(
      `add-previewLarge_${this.variantId}`,
    );
  }
  ...
  initEventListeners() {
    if (this.editModal) {
      document
        .getElementById(`add-modalClose_${this.variantId}`)
        .addEventListener("click", () => this.closeModal());
      document
        .getElementById(`add-cancelBtn_${this.variantId}`)
        .addEventListener("click", () => this.closeModal());
    }
    if (this.imageInput) {
  ...
  createModalIfNeeded() {
    if (!this.editModal) {
      const template = document
        .getElementById("add-editModalTemplate")
...
class addProductManager {
  ...
  fetchVariants() {
    fetch("/admin/variant/variants")
      ...
      .then((data) => {
        ...
        data.forEach((variantData) => {
          ...
          if (variantData.images && Array.isArray(variantData.images)) {
            imageManager.images = new Array(
              Math.max(variantData.images.length, 3),
            ).fill(null);
      ...
      .catch((error) => {
        ...
        Swal.fire(
          "Error",
          "Failed to load variants. Please refresh the page.",
          "error",
        );
      });
  }
  ...
  saveVariant(variantId) {
    ...
    const saveButton = document.querySelector(
      `#add-variant_${variantId} .add-save-variant`,
    );
    if (saveButton) {
  ...
  initColorPickerEvents() {
    ...
    document.getElementById("add-closeColorPicker").onclick = () =>
      modal.classList.remove("active");
    document.getElementById("add-cancelColorPicker").onclick = () =>
      modal.classList.remove("active");
        ...
        (variant) => `
            <tr><td><div class="add-variant-color" id="add-variant_${variant.id}">
                ...
                <div class="add-color-header">
                    ...
                    <button type="button" class="add-save-variant" data-product-action="save-variant" data-variant-id="${variant.id}">Save</button>
                    <button type="button" class="add-remove-variant" data-product-action="remove-variant" data-variant-id="${variant.id}">Remove</button>
                </div>
                        ...
                        <div class="add-tags-section">
                            ...
                            <div class="add-tags-container">
                                ${(variant.tags || [])
                                  .map(
                                    ...
                                    <span class="add-tag">
                                        ${tag}
                                        <button type="button" data-product-action="remove-tag" data-variant-id="${variant.id}" data-tag-index="${index}" aria-label="Remove ${tag} tag">&times;</button>
                                    </span>
                                `,
                                  )
                                  .join("")}
                            ...
                            <div class="add-add-tag-form">
                                <input type="text" 
                                       ...
                                       data-tag-input="true"
                                       data-variant-id="${variant.id}">
                                <button type="button" data-product-action="add-tag" data-variant-id="${variant.id}">Add</button>
                            </div>
                        </div>
                    </div>
                </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (button.dataset.productAction === "save-variant") {
      productManager.saveVariant(variantId);
    } else if (button.dataset.productAction === "remove-variant") {
      productManager.removeVariant(variantId);
    } else if (button.dataset.productAction === "remove-tag") {
      productManager.removeTag(variantId, Number(button.dataset.tagIndex));
    } else if (button.dataset.productAction === "add-tag") {
      const input = button.previousElementSibling;
      productManager.addTag(variantId, input.value.trim());
      input.value = "";
    }
  });

  document.addEventListener("change", (event) => {
    if (!event.target.matches("[data-stock-input]")) return;
    productManager.updateStock(
      event.target.dataset.variantId,
      Number(event.target.dataset.sizeIndex),
      event.target.value,
    );
  });

  document.addEventListener("keydown", (event) => {
    if (event.key !== "Enter" || !event.target.matches("[data-tag-input]"))
      return;
    event.preventDefault();
    productManager.addTag(
      event.target.dataset.variantId,
