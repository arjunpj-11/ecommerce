...
function makeUserId() {
  return (
    "usr_" + Math.random().toString(36).substr(2, 9) + Date.now().toString(36)
  );
}
...
const MAIN_CATEGORIES = [
  {
    ...
    image:
      "https://images.unsplash.com/photo-1490578474895-699bc4e2cf59?w=800&auto=format&fit=crop&q=80",
    offerPercentage: 15,
    status: "active",
  },
  {
...
const PRODUCTS_DATA = [
  // ── Men T-Shirts ──────────────────────────────────────────────────────────
  {
    ...
    description:
      "Elevate your daily rotation with the Aura Supima Cotton Tee. Crafted from 100% extra-long staple Supima cotton for unparalleled softness, enhanced color retention, and exceptional drape.",
    price: 1499,
    discountPrice: 1199,
    subCategoryName: "T-Shirts",
    review: 5,
  ...
  },
  {
    ...
    description:
      "Designed for modern urban aesthetic, this 280 GSM heavyweight cotton tee features drop shoulders, structured fit, and subtle silicone badge detailing on the cuff.",
    price: 1899,
    discountPrice: 1499,
    subCategoryName: "T-Shirts",
    review: 5,
  ...
  // ── Men Jackets ───────────────────────────────────────────────────────────
  {
    ...
    description:
      "Handcrafted from full-grain lambskin leather with matte silver hardware and quilted viscose lining. Features asymmetric zip closure and reinforced shoulder panels for iconic style.",
    price: 9999,
    discountPrice: 7999,
    subCategoryName: "Jackets",
    review: 5,
  ...
  // ── Men Jeans ─────────────────────────────────────────────────────────────
  {
    ...
    description:
      "Woven in Italy with premium stretch ring-spun cotton denim. Finished with hand-distressed whiskering and custom branded hardware for superior durability.",
    price: 3499,
    discountPrice: 2799,
    subCategoryName: "Jeans",
    review: 5,
  ...
  // ── Women Dresses ─────────────────────────────────────────────────────────
  {
    ...
    description:
      "Turn heads in the Siren Silk Wrap Dress. Tailored from lustrous mulberry silk blend with a flattering V-neckline, cascading waist tie, and subtle side slit.",
    price: 4999,
    discountPrice: 3899,
    subCategoryName: "Dresses",
    review: 5,
...

  {
    ...
    description:
      "Flowy, feminine, and ethereal. Crafted from lightweight pleated chiffon featuring hand-painted floral motifs, balloon sleeves, and a tiered A-line skirt.",
    price: 3799,
    discountPrice: 2999,
    subCategoryName: "Dresses",
    review: 5,
  ...
  // ── Women Tops ────────────────────────────────────────────────────────────
  {
    ...
    description:
      "Chic tailored corset top made from breathable European linen blend. Features flexi-boning for structured silhouette and delicate sweet-heart neckline.",
    price: 2199,
    discountPrice: 1699,
    subCategoryName: "Tops",
    review: 4,
  ...
  // ── Women Sarees ──────────────────────────────────────────────────────────
  {
    ...
    description:
      "Woven with pure Mulberry silk and gold zari threads. Inspired by temple architecture motifs, completed with an unstitched blouse piece.",
    price: 12999,
    discountPrice: 9999,
    subCategoryName: "Sarees",
    review: 5,
  ...
  // ── Footwear Sneakers ─────────────────────────────────────────────────────
  {
    ...
    description:
      "Minimalist luxury court sneakers featuring Italian full-grain leather upper, padded collar, and durable cupsole construction with memory foam insoles.",
    price: 5999,
    discountPrice: 4499,
    subCategoryName: "Sneakers",
    review: 5,
...

  {
    ...
    description:
      "Next-gen sock-knit high-top sneakers with responsive EVA midsole cushioning and sculptural rubber tread for ultimate lightweight agility.",
    price: 4999,
    discountPrice: 3699,
    subCategoryName: "Sneakers",
    review: 4,
  ...
  // ── Kids ──────────────────────────────────────────────────────────────────
  {
    ...
    description:
      "Super soft GOTS-certified organic cotton set for active toddlers. Includes stretch waistband joggers and snap-closure graphic pullover.",
    price: 1599,
    discountPrice: 1199,
    subCategoryName: "Boys Wear",
    review: 5,
...
const BANNERS_DATA = [
  {
    ...
    subtext:
      "Discover refined silhouettes, breathable fabrics, and effortless elegance.",
    buttonText: "Explore Collection",
    categoryType: "MainCategory",
    categoryName: "Men",
    imageUrl:
      "https://images.unsplash.com/photo-1490578474895-699bc4e2cf59?w=1600&auto=format&fit=crop&q=80",
    order: 1,
    isActive: true,
  },
  {
    ...
    subtext:
      "Handcrafted Italian sneakers, boots, and timeless court classics.",
...
const COUPONS_DATA = [
  ...
  {
    couponName: "Festive Season Special",
    couponCode: "FESTIVE20",
    discount: 20,
    minAmount: 1499,
    validity: new Date(Date.now() + 180 * 24 * 60 * 60 * 1000),
    status: "Active",
  },
  {
    couponName: "Summer Launch Sale",
    couponCode: "SUMMER15",
    discount: 15,
    minAmount: 999,
  ...
  {
    couponName: "VIP Club Member",
    couponCode: "VIP25",
    discount: 25,
    minAmount: 2999,
    validity: new Date(Date.now() + 120 * 24 * 60 * 60 * 1000),
    status: "Active",
  },
];
...
async function runSeed() {
  ...
  try {
    ...
    for (const sc of SUB_CATEGORIES) {
      ...
      if (!mainId) {
        warn(
          `Main category "${sc.mainCategoryName}" not found for subcategory "${sc.subCategoryName}"`,
        );
        continue;
      }
    ...
    for (const prodData of PRODUCTS_DATA) {
      ...
      if (!subId) {
        warn(
          `Subcategory "${prodData.subCategoryName}" not found for product "${prodData.name}"`,
        );
        continue;
      }
      ...
      log(
        `Product: "${prodData.name}" — ${prodData.variants.length} color variants, ${prodData.variants.length * 4} images`,
      );
    }
    ...
    for (const ud of USERS_DATA) {
      ...
      if (Wallet) {
        const initBal = ud.email.includes("priya")
          ? 250
          : ud.email.includes("rahul")
            ? 500
            : ud.email.includes("anjali")
              ? 150
              : 0;
        await Wallet.create({
          user: userDoc._id,
    ...
    console.table([
      {
        ...
        Count: await MainCategory.countDocuments(),
      },
      {
