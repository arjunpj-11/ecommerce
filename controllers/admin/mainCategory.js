...
exports.postAdminCategoryTable = async (req, res, next) => {
  try {
    ...
    let subcategoriesCount = await SubCategory.aggregate([
      { $group: { _id: "$mainCategory", count: { $sum: 1 } } },
    ]).exec();
    res.render("../views/partials/admin/mainCategoryTable", {
      mainCategories,
      subcategoriesCount,
    });
  } catch (error) {
          ...
          { _id: product._id },
          {
            $set: {
              discountPrice: parseFloat(
                (product.price - (product.price * offer) / 100).toFixed(0),
              ),
            },
          },
        );
...
exports.getAdminCategorySearch = async (req, res, next) => {
  try {
    let mainCategories = await MainCategory.find({
      mainCategoryName: { $regex: `${req.query.value}`, $options: "i" },
    });
    let subcategoriesCount = await SubCategory.aggregate([
      { $group: { _id: "$mainCategory", count: { $sum: 1 } } },
    ]).exec();
...
exports.postAdminCategoryEdit = async (req, res, next) => {
  try {
    ...
    const exist = await MainCategory.findOne({
      mainCategoryName: name,
      _id: { $ne: id },
    });
    if (exist) {
