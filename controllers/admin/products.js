...
const productController = {
  ...
  getHomePage: async (req, res, next) => {
    try {
      ...
      const page = 1;
      const limit = 10;
      const query = req.session.productId
        ? { subCategory: req.session.productId }
        : {};

      const [mainCategories, subCategories, totalProducts, products] =
        await Promise.all([
          mainCategory.find(),
          subCategory.find(),
  ...
  getProductsTable: async (req, res, next) => {
    try {
      ...
      // Build base query
      let query = req.session.productId
        ? { subCategory: req.session.productId }
        : {};
      ...
      const [mainCategories, subCategories, totalProducts, productsBasic] =
        await Promise.all([
      ...
      const products = await Promise.all(
        productsBasic.map(async (product) => {
          ...
          return {
            ...product,
            displayImage:
              variant?.images?.[0] || product.image || "/api/placeholder/48/48",
          };
        }),
      );
  ...
  getAddProductPage: async (req, res, next) => {
    try {
      ...
      res.render("../views/pages/admin/addProducts", {
        mainCategories,
        subCategories,
      });
    } catch (error) {
  ...
  activateProduct: async (req, res, next) => {
    try {
      if (req.session.subCategoryStatus === "inactive") {
        return res.json({
          success: false,
          message: "Please activate the subcategory first",
        });
      }
