...
const salesController = {
    ...
    try {
      const dateFilter = {
        orderDate: {
          ...
          $lte: new Date(
            new Date(endDate).setUTCDate(new Date(endDate).getUTCDate() + 1),
          ),
        },
        status: "Delivered",
      };
      ...
      const productIds = [
        ...new Set(salesData.map((item) => item._id.productId)),
      ];
      ...
      const transformedData = products.map((product) => {
        ...
        return {
          ...
          name: product.name,
          image:
            productVariants.length > 0 ? productVariants[0].images[0] : null,
          totalQuantity: productSales.reduce(
            (sum, sale) => sum + sale.quantity,
            0,
          ),
          totalAmount: productSales.reduce(
      ...
      const stats = {
        ...
        totalUnits: transformedData.reduce(
  ...
  downloadCSV: async (res, salesData, startDate, endDate) => {
    const flattenedData = salesController.flattenSalesData(
      salesData,
      startDate,
      endDate,
    );
  ...
  downloadPDF: async (res, salesData, startDate, endDate) => {
    ...
    salesData.products.forEach((product) => {
      ...
      doc
        .text(product.productId, columns.productId.x, y)
