...
exports.getHomePage = async (req, res, next) => {
  try {
    ...
    res.render("../views/pages/index/search", {
      search,
      authentication,
      categoriesWithSubs,
    });
...
exports.getSearchRecommendations = async (req, res, next) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error fetching recommendations.", details: err.message }); // Handle server error
  }
};
...
exports.getProducts = async (req, res, next) => {
  try {
    ...
    const categoriesArray = Array.isArray(categories)
      ? categories
      : [categories].filter(Boolean); // Ensure categories is an array
    ...
    if (categoriesArray.length > 0) {
      const categoryIds = categoriesArray.map(
        (cat) => new mongoose.Types.ObjectId(cat),
      );
      aggregationPipeline.push({
        $match: {
    ...
    res
      .status(500)
      .json({ error: "Error fetching products.", details: err.message }); // Handle server error
  }
};
