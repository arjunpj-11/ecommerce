...
const homeController = {
  ...
  getHomePage: async (req, res) => {
    try {
      ...
      const topCategories = await MainCategory.find({ status: "active" }).limit(
        categoryLimit,
      );
      ...
      const topSoldItems = await Product.aggregate([
        ...
        },
        {
          $addFields: {
            ...
            variantId: {
              $ifNull: [{ $arrayElemAt: ["$variants._id", 0] }, null],
            },
          },
        },
        { $sort: { orderCount: -1 } },
      ...
      res
        .status(500)
        .render("error", { message: "Error loading home page", error }); // Handle errors
    }
  },
};
